...
const Profile = props => {
    ...
    const handleClick = async() => {
        try{
            const requestBody = JSON.stringify({id: user.id, token: user.token, username: username, birthday: birthday});
    ...
    return (
        ...
        {auth &&
        <Button onClick={() => setEdit(!edit)}>{!edit ? 'Edit' : 'Go Back'}</Button> 
        } 
    </div>
    </div>
    </BaseContainer>
...
const Profiless = props => {
    ...
    function renderTags() {
        if (editMode === true) return <ProfileOnEdit>
            {user, switchToEdit}
        </ProfileOnEdit>;
        return (
